...
# MODIFICATION A: Detect HAOS mode for logging behavior
...
# MODIFICATION B: Skip directory creation in HAOS mode
setup_logging() {
    # In HAOS mode, logging goes to stdout, no directory needed
...
# MODIFICATION B: Write to stdout in HAOS mode, file otherwise
log_state() {
    local cpu_temp=$1
    local cpu_state=$2
    local nvme_temp=$3
...
# MODIFICATION B: Always write to stderr (both modes)
log_error() {
    local message=$1
    local timestamp=$(date '+%Y-%m-%d %H:%M:%S')

    # Always output errors to stderr (both modes)
    echo "$timestamp | ERROR: $message" >&2

    # Additionally log to file if not in HAOS mode
...
log_debug() {
    if [[ "$DEBUG_MODE" == "1" ]]; then
        ...
        if (( LOG_TO_STDOUT == 1 )); then
            ...
            # File mode: both file and stderr
            local logfile=$(get_log_file)
